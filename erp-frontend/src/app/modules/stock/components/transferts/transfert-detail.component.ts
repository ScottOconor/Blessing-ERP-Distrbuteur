...
import { StockService, StockPicking, StockPickingType, StockLocation, Product, Warehouse } from '../../services/stock.service';
...
export class TransfertDetailComponent implements OnInit {
  ...
  filterSrcWarehouse: number | '' = '';
  filterDestWarehouse: number | '' = '';
  ...
  get filteredSrcLocations(): StockLocation[] {
    if (!this.filterSrcWarehouse) return this.internalLocations;
    return this.internalLocations.filter(l => l.warehouseId === Number(this.filterSrcWarehouse));
  }

  get filteredDestLocations(): StockLocation[] {
    if (!this.filterDestWarehouse) return this.internalLocations;
    return this.internalLocations.filter(l => l.warehouseId === Number(this.filterDestWarehouse));
  }

  addLine(): void { this.lines.push({ productId: null, qtyDemanded: 1, qtyDone: 1, priceUnit: 0, uomName: '', availableQty: 0 }); }
  ...
  buildReq() {
    return {
      ...
      origin: this.origin, notes: this.notes, scheduledDate: this.scheduledDate, companyId: this.companyId,
  ...
  save(): void {
    if (!this.selectedSrcId || !this.selectedDestId || this.lines.filter(l => l.productId).length === 0) {
      this.errorMsg = 'Source, destination et au moins une ligne obligatoires'; return;
    }
    if (this.selectedSrcId === this.selectedDestId) { this.errorMsg = 'Source et destination doivent être différentes'; return; }
  ...
  stateLabel(s: string): string { return { draft: 'Brouillon', done: 'Effectué', cancelled: 'Annulé' }[s] || s; }
}
